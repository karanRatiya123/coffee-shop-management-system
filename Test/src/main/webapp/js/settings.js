...
function loadSessionOperatorInfo() {
    ...
    if (sessionAvatar && avatarEl) {
        const fixed = (sessionAvatar === 'images/default-avatar.png')
            ? 'images/default-avatar.svg'
            : sessionAvatar;
        avatarEl.style.backgroundImage = `url('${fixed}')`;
    }
...
function updateSettingsOperatorDOM(operator) {
    ...
    const rawAvatar = operator.avatar || sessionStorage.getItem('operatorAvatar') || 'images/default-avatar.svg';
    const avatar = (rawAvatar === 'images/default-avatar.png' || !rawAvatar)
        ? 'images/default-avatar.svg'
        : rawAvatar;
