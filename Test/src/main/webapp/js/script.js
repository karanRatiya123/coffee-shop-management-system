...
// Resolve avatar path from DB into a URL the browser can load.
// Common mistake: DB stores images/default-avatar.png (file does not exist).
function resolveAvatarUrl(avatar) {
    if (!avatar || typeof avatar !== 'string') {
        return 'images/default-avatar.svg';
    }
    let path = avatar.trim().replace(/\\/g, '/');
    if (!path) return 'images/default-avatar.svg';
    // Strip accidental Windows absolute / file paths down to filename under images/
    if (/^[a-zA-Z]:\//.test(path) || path.startsWith('file:')) {
        path = path.split('/').pop();
    }
    if (path === 'default-avatar.png' || path === 'images/default-avatar.png') {
        return 'images/default-avatar.svg';
    }
    // Bare filename → images/<file>
    ...
    try {
        ...
        if (Array.isArray(operators) && operators.length > 0) {
            operators = operators.map(op => ({
                ...op,
                avatar: resolveAvatarUrl(op.avatar)
            }));
...
function updateSelectedOperator(operator) {
    ...
    const avatarUrl = resolveAvatarUrl(operator.avatar);
...
function populateOperatorDropdown() {
    ...
    operators.forEach((operator, index) => {
        ...
        const safeRole = (operator.role || '').replace(/'/g, "\\'");
...
async function evaluatePinCode() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            sessionStorage.setItem('operatorAvatar', resolveAvatarUrl(selectedOperator.avatar));
